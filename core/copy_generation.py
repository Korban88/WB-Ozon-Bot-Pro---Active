...
# Тональность по категории — применяется в листинге и рекламном копипаке
_CATEGORY_TONE = {
    "clothing":    "Тон: модный, образный, живой язык. Акцент на look и ощущении.",
    "electronics": "Тон: технически точный, конкретные характеристики, без воды. Факты > эмоции.",
    "beauty":      "Тон: чувственный, нежный. Акцент на ощущениях, результате, ритуале ухода.",
    "home":        "Тон: уютный, тёплый. Образы домашнего комфорта, качества жизни.",
    "accessories": "Тон: статусный, изысканный. Детали craftsmanship, finish, исполнение.",
    "other":       "Тон: деловой, чёткий. Фокус на практической пользе.",
}


def _tone(category: str) -> str:
    return _CATEGORY_TONE.get(category, _CATEGORY_TONE["other"])
...
# ─── UGC сценарий (Hook / Micro-story / Benefit / CTA) ───────────────────────

_UGC_SYSTEM = (
    "Ты — режиссёр UGC-контента для e-commerce. "
    "Пишешь сценарии коротких видео так, как говорят живые люди — НЕ как реклама. "
    "Структура: Hook (захват внимания) → Micro-story (личный момент) → Benefit (польза) → CTA (мягкий). "
    "Никаких шаблонных фраз типа 'Привет, сегодня расскажу...'. Речь разговорная, современная, искренняя. "
    "Работаешь ТОЛЬКО с предоставленными фактами. "
    "Ответ СТРОГО в JSON без markdown."
)
...
def _ugc_prompt(p: ProductData) -> str:
    return f"""Сценарий UGC-ролика (Hook → Micro-story → Benefit → CTA).

{_facts_inventory(p)}
{_HARD_RULE}

Структура:
- hook: визуальный или вербальный захват первых 2-3 сек. НЕ "Привет, сегодня расскажу". Нестандартно.
- micro_story: личный момент/ситуация где этот товар решил задачу (1-2 предложения, живым языком)
- benefit: что конкретно даёт товар — одна фраза из фактов (НЕ выдуманная)
- cta: мягкий призыв — НЕ "переходите по ссылке". Что-то живое (2-3 слова)
- scenes: раскадровка по временным меткам

Верни JSON:
{{
  "duration":    "15-30 сек",
  "format":      "9:16 вертикальное",
  "hook":        "действие или фраза для первых 2-3 сек",
  "micro_story": "ситуация или личный момент — 1-2 предложения",
  "benefit":     "конкретная польза из фактов — 1 живая фраза",
  "cta":         "мягкий финальный призыв",
  "scenes": [
